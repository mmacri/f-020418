...
import { Users, TrendingUp, TrendingDown, Zap, Download, Trash2, AlertTriangle } from 'lucide-react';
...
const GeneralStats: React.FC<GeneralStatsProps> = ({ timeframe, setTimeframe }) => {
  ...
  const confirmClearStats = async () => {
    try {
      ...
      toast("Failed to clear statistics. Please try again.");
    } finally {
  ...
  const handleExportStats = (exportPeriod?: 'week' | 'month' | 'quarter' | 'all') => {
    try {
      ...
      toast(`General statistics for ${periodName.toLowerCase()} have been exported successfully.`);
    } catch (error) {
